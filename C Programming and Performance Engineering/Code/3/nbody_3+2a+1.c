...
real3 bodyBodyInteraction(real4 iPos, real4 jPos)
{
  ...
  if (distSqr < SOFTENING_SQUARED) s = jPos.w / POW(SOFTENING_SQUARED,1.5);
  else                             s = jPos.w / POW(distSqr,1.5);
  ...
  for (i = 0; i < n; i++)
  {
    ...
    for (j = 0; j < n; j++)
      //if (i!=j)
      {
        ...
        distSqr = rx*rx+ry*ry+rz*rz;
// new code
        if (distSqr < SOFTENING_SQUARED) s = SQRT(SOFTENING_SQUARED)*SOFTENING_SQUARED;
        else                             s = SQRT(distSqr)*distSqr;
